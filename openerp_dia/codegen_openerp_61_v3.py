...
#    OpenERP (<http://tiny.be>) by Tiny SPRL Copyright (C) 2004-2010
    ...
    def view_class_get(self, cn, cd):
        ...
        for sa,attr in cd.attributes:
            ...
            if attr[0] in ('one2many', 'many2many', 'text'):
                ...
                elif field_label.startswith(' \''):
                    field_label = field_label[2:]
                if field_label.endswith('\''):
                    field_label = field_label[:-1]
                elif field_label.endswith('\' '):
                    field_label = field_label[:-2]
                fields_form += ("                <separator string=\"%s\" colspan=\"4\"/>\n") % (field_label or 'Unknown')
            fields_form += ("                <field name=\"%s\" "+attrs+"select=\"%d\"/>\n") % (sa,i)
            # v3. Por alguna razón el codegen original no añade por defecto los o2m y m2m a la vista (?)
            # Comentamos el if y reidentamos fields_tree
...
dia.register_export ("PyDia Code Generation (OpenERP 6.1 v3)", "zip", OpenERPRenderer())
